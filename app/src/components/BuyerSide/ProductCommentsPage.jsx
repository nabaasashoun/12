...
const ProductCommentsPage = () => {
    ...
    return (
      <div className={`p-6 max-w-6xl mx-auto min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="text-center py-12">
          ...
          <p className={`mt-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading product reviews...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className={`p-6 max-w-6xl mx-auto min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className={`rounded-lg p-4 flex items-center ${isDarkMode ? 'bg-red-900/30 border border-red-800' : 'bg-red-50 border border-red-200'}`}>
          <AlertCircle className={`w-5 h-5 mr-3 ${isDarkMode ? 'text-red-400' : 'text-red-500'}`} />
  ...
  return (
    <div className={`p-4 md:p-6 max-w-6xl mx-auto min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="mb-6">
        <div className="flex items-center mb-4">
          ...
          <p className={`text-[23px] font-bold ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>Product Reviews</p>
        </div>
      </div>
          ...
          <BuyerCardContent className="p-4">
            <div className="flex flex-col md:flex-row items-start md:items-center">
              ...
              <div className="flex-1">
                <h2 className={`text-lg font-semibold mb-1 ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>
                  {product.name}
                </h2>
                ...
                <div className="flex items-center">
                  <div className="flex items-center mr-4">
                    ...
                    <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-black'}`}>{averageRating}</span>
                    <span className={`text-sm ml-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                      ({totalComments} reviews)
                    </span>
                  </div>
                  <span className={`text-lg font-bold ${isDarkMode ? 'text-green-400' : 'text-blue-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <BuyerCard>
            <BuyerCardContent className="p-4">
              <h3 className={`font-semibold mb-4 ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>Rating Summary</h3>
                  ...
                  return (
                    <div key={star} className="flex items-center">
                      <div className="flex items-center w-16">
                        <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} w-4`}>{star}</span>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <div className="text-center">
                  <div className={`text-3xl font-bold mb-1 ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>
                    {averageRating}
                  </div>
          ...
          <BuyerCard>
            <BuyerCardContent className="p-4">
              <h3 className={`font-semibold mb-4 ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>
              ...
              {userComment && !editingCommentId ? (
                <div className="space-y-3">
                  ...
                  <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{userComment.comment}</p>
                  <div className="flex space-x-2">
                    <button
                      onClick={() => startEditing(userComment)}
                ...
                <form onSubmit={editingCommentId ? handleUpdateComment : handleSubmitComment}>
                  ...
                  <div className="mb-4">
                    ...
                    <textarea
                      ...
                      className={`w-full p-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-h-[120px] ${
                        isDarkMode 
        ...
        <div className="lg:col-span-2">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
            <h2 className={`text-lg font-semibold mb-2 md:mb-0 ${isDarkMode ? 'text-gray-100' : 'text-black'}`}>
                ...
                <BuyerCard key={comment.id}>
                  <BuyerCardContent className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                          {comment.user_photo ? (
                            <img
                        ...
                        <div>
                          <p className={`font-medium ${isDarkMode ? 'text-gray-200' : 'text-black'}`}>
                            {comment.user_name}
                          </p>
                          <div className="flex items-center text-sm">
                            ...
                            <Clock className={`w-3 h-3 mr-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                      ...
                      {comment.is_own_comment && !editingCommentId && (
                        <div className="flex space-x-2">
                          <button
                            ...
                            className={`p-1 ${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-400 hover:text-blue-500'}`}
                          >
                            <Edit className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => handleDeleteComment(comment.id)}
                            className={`p-1 ${isDarkMode ? 'text-gray-400 hover:text-red-400' : 'text-gray-400 hover:text-red-500'}`}
                          >
                    ...
                    <p className={`mb-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{comment.comment}</p>
                  </BuyerCardContent>
                </BuyerCard>
              ))}
            </div>
